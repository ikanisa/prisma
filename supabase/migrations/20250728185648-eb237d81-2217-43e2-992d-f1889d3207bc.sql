-- QR Payments System Migration (Final Clean Version)
...
-- Create unique RLS Policies for payments
DO $$
BEGIN
    ...
    CREATE POLICY "payments_users_select_own"
        ON public.payments FOR SELECT
        USING (user_id = auth.uid());

    CREATE POLICY "payments_users_insert_own"
        ON public.payments FOR INSERT
        WITH CHECK (user_id = auth.uid());

    CREATE POLICY "payments_users_update_own"
        ON public.payments FOR UPDATE
        USING (user_id = auth.uid() AND status = 'pending');

    CREATE POLICY "payments_system_all"
...
-- Create unique RLS Policies for payment_events
DO $$
BEGIN
    DROP POLICY IF EXISTS "payment_events_users_select_own" ON public.payment_events;
    DROP POLICY IF EXISTS "payment_events_system_all" ON public.payment_events;
    DROP POLICY IF EXISTS "payment_events_admin_all" ON public.payment_events;
    
    CREATE POLICY "payment_events_users_select_own"
        ON public.payment_events FOR SELECT
        USING (payment_id IN (SELECT id FROM public.payments WHERE user_id = auth.uid()));

    CREATE POLICY "payment_events_system_all"
...
BEGIN
    ...
    IF p_ref IS NULL THEN
        p_ref := 'EM' || TO_CHAR(NOW(), 'YYMMDD') || LPAD(FLOOR(RANDOM() * 10000)::TEXT, 4, '0');
    END IF;
