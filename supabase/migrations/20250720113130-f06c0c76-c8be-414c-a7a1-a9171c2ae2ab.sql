-- Step 8A: Security Audit & Critical Fixes (Fixed)
-- Fix RLS disabled tables and security definer functions

-- Fix RLS disabled in public schema, excluding system tables
DO $$
DECLARE
    table_record record;
    system_tables TEXT[] := ARRAY['spatial_ref_sys', 'geography_columns', 'geometry_columns', 'raster_columns', 'raster_overviews'];
BEGIN
...
CREATE TYPE IF NOT EXISTS public.app_role AS ENUM ('admin', 'moderator', 'user', 'driver', 'farmer', 'business_owner');
...
-- Create secure helper functions for roles with proper search paths
