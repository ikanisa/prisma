-- Comprehensive Accounting Knowledge Base Schema
-- Supports IFRS, IAS, ISA, GAAP, Tax Laws, ACCA, CPA materials
-- with pgvector embeddings for RAG (Retrieval-Augmented Generation)
...
comment on table jurisdictions is 'Geographical/regulatory jurisdictions for accounting and tax standards';
comment on column jurisdictions.code is 'ISO-style jurisdiction code (RW, US, EU, GLOBAL, etc.)';

-- 3) Knowledge sources (IFRS Foundation, RRA, ACCA, etc.)
create table if not exists knowledge_sources (
    id uuid primary key default gen_random_uuid(),
    name text not null,                         -- e.g. "IFRS Foundation", "Rwanda Revenue Authority"
...
comment on table knowledge_sources is 'Authoritative sources of accounting/tax knowledge (standards bodies, regulators)';
comment on column knowledge_sources.authority_level is 'PRIMARY = law/standard, SECONDARY = guidance, INTERNAL = company policy';
...
comment on table knowledge_documents is 'Individual accounting standards, tax laws, guidance documents';
...
-- 5) Knowledge chunks (RAG text units)
...
comment on table knowledge_chunks is 'Text chunks for RAG retrieval, chunked from knowledge_documents';
comment on column knowledge_chunks.section_path is 'Hierarchical reference (e.g., IAS 21.8-12)';
...
-- 6) Knowledge embeddings (vector search)
...
comment on table knowledge_embeddings is 'Vector embeddings for semantic search over knowledge_chunks';

-- Use ivfflat index for cosine similarity search
...
comment on table ingestion_jobs is 'Track ingestion pipeline runs for audit and monitoring';

create index if not exists idx_ingestion_jobs_status
    on ingestion_jobs (status, created_at desc);

-- 8) Ingestion files (per PDF, HTML, etc.)
...
comment on table ingestion_files is 'Individual files within ingestion jobs';
...
-- 9) Agent queries log (audit trail for DeepSearch, AccountantAI)
create table if not exists agent_queries_log (
    ...
    top_chunk_ids uuid[],                     -- the chunks used in response
    jurisdiction_id uuid references jurisdictions(id),
    created_at timestamptz not null default now(),
    latency_ms integer,
    metadata jsonb default '{}'::jsonb
...
comment on table agent_queries_log is 'Audit log of agent knowledge retrieval queries';
