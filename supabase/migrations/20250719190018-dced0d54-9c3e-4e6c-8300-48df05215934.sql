...
-- Create model registry for fallbacks
...
-- Create centralized documents with OpenAI file support
ALTER TABLE public.centralized_documents ADD COLUMN IF NOT EXISTS openai_file_id TEXT;
ALTER TABLE public.centralized_documents ADD COLUMN IF NOT EXISTS file_purpose TEXT DEFAULT 'assistants';
...
-- RLS Policies for model registry
CREATE POLICY "Admin can manage model registry" ON public.model_registry
  FOR ALL USING (is_admin());
...
INSERT INTO public.model_registry (task_name, primary_model, secondary_model, fallback_model) VALUES
('conversation', 'gpt-4o', 'gpt-4o-mini', 'gpt-3.5-turbo'),
('transcription', 'whisper-1', NULL, NULL),
('vision', 'gpt-4o', NULL, NULL),
('quality_check', 'gpt-4o-mini', NULL, NULL)
ON CONFLICT DO NOTHING;