-- ===============================================
-- easyMO Driver System - Complete Implementation
-- ===============================================
...
-- Create trips table for ride management
CREATE TABLE IF NOT EXISTS trips (
  ...
  price int,
  status text DEFAULT 'assigned' CHECK (status IN ('assigned', 'accepted', 'picked_up', 'delivered', 'cancelled')),
  created_at timestamptz DEFAULT now(),
...
-- Create bookings table for trip management
...
CREATE POLICY "Drivers read own trips" ON trips
  FOR SELECT USING (driver_id IN (SELECT id FROM drivers WHERE user_id = auth.uid()));
...
CREATE POLICY "Admin manage trips" ON trips
  FOR ALL USING (is_admin()) WITH CHECK (is_admin());
...
CREATE POLICY "System manage trips" ON trips
  FOR ALL USING (true) WITH CHECK (true);
