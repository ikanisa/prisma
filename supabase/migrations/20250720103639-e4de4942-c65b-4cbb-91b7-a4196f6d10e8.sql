-- STEP 5: WhatsApp Integration Enhancement - Database Schema
-- Add tables for enhanced WhatsApp message tracking and template management

-- WhatsApp Templates table (if not exists)
CREATE TABLE IF NOT EXISTS public.whatsapp_templates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  template_name TEXT NOT NULL UNIQUE,
  category TEXT NOT NULL CHECK (category IN ('AUTHENTICATION', 'MARKETING', 'UTILITY', 'TRANSACTIONAL')),
...
-- Admin access to WhatsApp management
CREATE POLICY "Admin can manage WhatsApp templates" 
ON public.whatsapp_templates FOR ALL 
USING (is_admin())
WITH CHECK (is_admin());

CREATE POLICY "System can access WhatsApp templates" 
