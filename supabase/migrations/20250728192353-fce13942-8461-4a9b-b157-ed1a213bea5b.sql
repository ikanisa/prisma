...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='learning_gap_instances' AND column_name='gap_type') THEN
    ALTER TABLE public.learning_gap_instances ADD COLUMN gap_type TEXT;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='learning_gap_instances' AND column_name='gap_category') THEN
    ALTER TABLE public.learning_gap_instances RENAME COLUMN gap_category TO gap_type;
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='learning_gap_instances' AND column_name='severity_level') THEN
