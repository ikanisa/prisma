...
-- Recreate conversation_summary view without SECURITY DEFINER
...
-- 2. Enable RLS on spatial_ref_sys if it exists and we have permission
-- Note: This table is typically managed by PostGIS, so we may not be able to enable RLS
-- We'll try, but expect this might fail for system tables

-- 3. Fix functions by adding search_path where missing
