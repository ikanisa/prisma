...
CREATE TABLE public.vehicle_listings (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  vendor_id UUID REFERENCES hardware_vendors(id),
  make TEXT NOT NULL,
  model TEXT NOT NULL,
  year INTEGER,
  price_rwf INTEGER,
...
CREATE POLICY "admin_manage_vehicle_listings" ON public.vehicle_listings FOR ALL USING (is_admin());
CREATE POLICY "admin_manage_property_sync_log" ON public.property_sync_log FOR ALL USING (is_admin());

-- Public can view active vehicle listings
CREATE POLICY "public_view_active_vehicles" ON public.vehicle_listings 
...
CREATE INDEX idx_vehicle_listings_featured ON public.vehicle_listings(featured);
...
CREATE INDEX idx_vehicle_listings_not_deleted ON public.vehicle_listings(id) WHERE deleted_at IS NULL;
