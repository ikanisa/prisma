...
create policy "admin_read" on whatsapp_messages
  for select using (auth.role() = 'service_role');
create policy "admin_write" on whatsapp_messages
  for insert using (auth.role() = 'service_role');

create policy "admin_read" on whatsapp_conversations
  for select using (auth.role() = 'service_role');
create policy "admin_write" on whatsapp_conversations
