...
-- 2. Enable RLS on spatial_ref_sys if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'spatial_ref_sys' AND table_schema = 'public') THEN
        ...
        -- Allow read access to spatial reference systems
        CREATE POLICY "Allow read access to spatial_ref_sys" ON public.spatial_ref_sys
        FOR SELECT USING (true);
    END IF;
EXCEPTION WHEN OTHERS THEN
    -- Ignore errors if table doesn't exist or RLS already enabled
    NULL;
END $$;

-- 3. Create enhanced security validation functions
...
-- 4. Add security event logging trigger
...
-- 5. Create function to check admin status securely
