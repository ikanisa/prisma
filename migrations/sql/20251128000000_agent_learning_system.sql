...
-- Learning, Training & Continuous Improvement
-- ============================================
...
-- RAG Embedding Training Pairs
CREATE TABLE IF NOT EXISTS embedding_training_pairs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    query TEXT NOT NULL,
    document TEXT NOT NULL,
    label VARCHAR(20) CHECK (label IN ('positive', 'hard_negative', 'negative')),
...
CREATE INDEX IF NOT EXISTS idx_learning_examples_org ON learning_examples(organization_id, is_active);
...
CREATE INDEX IF NOT EXISTS idx_annotations_expert ON expert_annotations(expert_id, created_at);
...
-- Learning Examples RLS
CREATE POLICY learning_examples_org_isolation ON learning_examples
    USING (organization_id = current_setting('app.current_organization_id', true)::uuid);

-- Agent Feedback RLS
CREATE POLICY agent_feedback_user_access ON agent_feedback
...
COMMENT ON TABLE learning_examples IS 'Core training data collected from user feedback, corrections, and expert demonstrations';
COMMENT ON TABLE agent_feedback IS 'Quick user ratings and feedback on agent executions';
COMMENT ON TABLE expert_annotations IS 'Expert-provided quality assessments and annotations';
COMMENT ON TABLE training_datasets IS 'Curated collections of learning examples for training';
COMMENT ON TABLE training_runs IS 'Records of training/optimization runs with metrics and artifacts';
COMMENT ON TABLE learning_experiments IS 'A/B tests for evaluating agent improvements';
COMMENT ON TABLE embedding_training_pairs IS 'Query-document pairs for fine-tuning RAG embeddings';
