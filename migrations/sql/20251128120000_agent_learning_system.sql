-- ============================================
-- AGENT LEARNING SYSTEM - DATABASE SCHEMA
-- Migration: 20251128120000_agent_learning_system.sql
-- Description: Core schema for AI agent learning, training & continuous improvement
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_learning_examples_status ON learning_examples(review_status) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_expert_annotations_example ON expert_annotations(learning_example_id);
CREATE INDEX IF NOT EXISTS idx_expert_annotations_expert ON expert_annotations(expert_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_experiments_status ON learning_experiments(status, created_at);

CREATE INDEX IF NOT EXISTS idx_dataset_examples_dataset ON dataset_examples(dataset_id);
CREATE INDEX IF NOT EXISTS idx_dataset_examples_example ON dataset_examples(example_id);
...
-- RLS Policies (assuming auth.uid() and organization context)
CREATE POLICY learning_examples_org_policy ON learning_examples
    FOR ALL USING (
        organization_id IS NULL OR 
        organization_id IN (SELECT id FROM organizations WHERE id = current_setting('app.current_organization_id')::uuid)
    );

CREATE POLICY agent_feedback_user_policy ON agent_feedback
    FOR ALL USING (user_id = auth.uid() OR EXISTS (
        SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('admin', 'expert')
    ));

CREATE POLICY expert_annotations_expert_policy ON expert_annotations
...
CREATE TRIGGER learning_examples_update_timestamp
    BEFORE UPDATE ON learning_examples
    FOR EACH ROW
    EXECUTE FUNCTION update_learning_example_timestamp();

CREATE TRIGGER training_datasets_update_timestamp
...
BEGIN
    ...
    SET 
        total_examples = (
            SELECT COUNT(*) FROM dataset_examples WHERE dataset_id = NEW.dataset_id
        ),
        updated_at = NOW()
...
CREATE TRIGGER dataset_examples_stats_update
    AFTER INSERT OR DELETE ON dataset_examples
    FOR EACH ROW
    EXECUTE FUNCTION update_dataset_statistics();

COMMENT ON TABLE learning_examples IS 'Core training examples collected from user feedback, corrections, and expert demonstrations';
COMMENT ON TABLE agent_feedback IS 'Quick user feedback and ratings for agent executions';
COMMENT ON TABLE expert_annotations IS 'Expert reviews and quality assessments of learning examples';
...
COMMENT ON TABLE learning_experiments IS 'A/B testing experiments for agent improvements';
