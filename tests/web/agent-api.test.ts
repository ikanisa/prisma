...
describe('agent approvals API routes', () => {
  ...
  afterEach(() => {
    ...
    it('forwards decision payload to agent service', async () => {
      process.env.NEXT_PUBLIC_API_BASE = 'https://api.prismaglow.example.com';
      ...
      expect(res.status).toBe(200);
      expect(fetchMock).toHaveBeenCalledWith('https://api.prismaglow.example.com/v1/approvals/approval-1/decision', {
        method: 'POST',
