...
def test_cors_allows_only_configured_origins(monkeypatch):
    ...
    monkeypatch.setenv('API_ALLOWED_ORIGINS', 'https://app.prismaglow.example.com')
    ...
    preflight_allowed = client.options(
        '/health',
        headers={
            'Origin': 'https://app.prismaglow.example.com',
            'Access-Control-Request-Method': 'GET',
        },
    )
    assert preflight_allowed.status_code in (200, 204)
    assert preflight_allowed.headers.get('access-control-allow-origin') == 'https://app.prismaglow.example.com'
