...
const ScannerStatusDisplay: React.FC<ScannerStatusDisplayProps> = ({
  ...
  if (scanStatus === "scanning") {
    return <div className="absolute left-1/2 bottom-[18vh] -translate-x-1/2 flex flex-col items-center">
        <LoadingSpinner />
        <span className="mt-2 text-base font-semibold text-white/90"></span>
      </div>;
  }
  ...
  if (scanStatus === "success" && scanResult) {
    return <div className="absolute left-1/2 bottom-[15vh] -translate-x-1/2 w-[90vw] max-w-lg flex items-center justify-center transition-all animate-fade-in">
        <button onClick={onUSSDPress} className="w-full py-5 px-6 text-2xl sm:text-3xl font-black rounded-3xl bg-gradient-to-r from-blue-500 via-indigo-500 to-blue-600 shadow-lg hover:scale-105 transition active:scale-95 text-white tracking-widest text-center ring-2 ring-blue-400/40 glow outline-none animate-pulse" style={{
        letterSpacing: "0.1em"
  ...
  if (scanStatus === "fail") {
    return <div className="absolute left-1/2 bottom-[15vh] -translate-x-1/2 w-[90vw] max-w-lg flex flex-col items-center transition-all animate-fade-in">
