...
export function buildTcwgMarkdown(template: string, params: TcwgTemplateParams) {
  ...
  return template
    ...
    .replace('{{ significant_findings }}', params.significantFindings ?? 'No significant findings beyond those already discussed.')
    ...
    .replace('{{ annex_gc_se }}', JSON.stringify({ goingConcern: params.goingConcern, subsequentEvents: params.subsequentEvents }, null, 2));
}
