import { hasSufficientRole, roleFromString, type AgentPlanDocument, type AgentPlanGenerationResult, type AgentPlanRefusal, type AgentPlanStep, type AgentPlanToolIntent, type AgentRequestContext, type GenerateAgentPlanInput } from './types';
...
function attachRequestedTools(steps: AgentPlanStep[], context?: AgentRequestContext) {
  ...
  const pending = context.requestedTools.filter((tool) => !knownTools.has(tool.toolKey));
  if (pending.length === 0) {
    return;
  }
...
function sanitisePlanDocument(payload: any, options: GenerateAgentPlanInput): AgentPlanDocument {
  ...
  const normalisedSteps: AgentPlanStep[] = rawSteps
    .map((step: any, index: number) => normaliseStep(step, index))
...
function normaliseStep(step: any, fallbackIndex: number): AgentPlanStep | undefined {
  if (!step || typeof step !== 'object') return undefined;
  const titleRaw = step.title ?? step.name ?? step.heading;
  const summaryRaw = step.summary ?? step.description ?? step.intent;
  if (typeof titleRaw !== 'string' || typeof summaryRaw !== 'string') {
    return undefined;
  }

  const stepIndex = typeof step.stepIndex === 'number' ? step.stepIndex : fallbackIndex;
  const exitCriteria = typeof step.exitCriteria === 'string' ? step.exitCriteria : undefined;
  const dependsOn = Array.isArray(step.dependsOn)
    ? step.dependsOn.filter((value: any) => Number.isInteger(value))
    : undefined;

  const toolIntents = Array.isArray(step.toolIntents)
    ? step.toolIntents
        .map((intent: any) => normaliseToolIntent(intent))
...
function normaliseToolIntent(intent: any): AgentPlanToolIntent | undefined {
  if (!intent || typeof intent !== 'object') return undefined;
  const toolKey = typeof intent.toolKey === 'string' ? intent.toolKey : typeof intent.key === 'string' ? intent.key : undefined;
  if (!toolKey) return undefined;

  const purposeRaw = intent.purpose ?? intent.reason ?? intent.summary;
  ...
  const minRole = roleFromString(intent.minRole ?? intent.min_role);

  return {
    toolKey,
    purpose,
    inputs,
    notes: typeof intent.notes === 'string' ? intent.notes : undefined,
