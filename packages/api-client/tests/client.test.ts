...
describe('ApiClient basic construction', () => {
  it('builds autonomy status url with query params', async () => {
    ...
    const client = new ApiClient({ baseUrl: 'https://api.prismaglow.example.com', fetch: mock });
    const res = await client.getAutonomyStatus('acme');
    expect((res as any).url).toBe('https://api.prismaglow.example.com/v1/autonomy/status?orgSlug=acme');
  ...
  it('posts release controls check payload', async () => {
    ...
    expect((res as any).url).toBe('https://api.prismaglow.example.com/api/release-controls/check');
  ...
  it('retries on 503 and then succeeds', async () => {
    ...
    const client = new ApiClient({ baseUrl: 'https://api.prismaglow.example.com', fetch: mock as any, retries: 2, retryDelayMs: 1 });
    const res = await client.getAutonomyStatus('acme');
