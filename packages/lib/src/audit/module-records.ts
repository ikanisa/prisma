...
async function fetchModuleRecord(
  ...
  const { data, error } = await client
    ...
    .select(
      'id, title, record_status, approval_state, current_stage, prepared_by_user_id, owner_user_id, current_reviewer_user_id, locked_at, approvals, metadata, created_by_user_id, updated_by_user_id',
    )
    .match({
      org_id: params.orgId,
...
export async function ensureAuditRecordApprovalStage(client: TypedClient, params: EnsureApprovalStageParams) {
