...
import { z } from 'zod';
import { computeMaltaCit } from '@/lib/tax/calculators';
...
const payloadSchema = z.object({
  ...
  carryForwardLosses: z.coerce.number().optional().default(0),
...
export async function POST(request: NextRequest) {
  ...
  const result = computeMaltaCit(inputs);
  const activity = recordActivity({
    ...
    metrics: {
      taxableIncome: result.metrics.taxableIncome,
      taxDue: result.metrics.taxDue,
    },
    actor: preparedBy,
  });
