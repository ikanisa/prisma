...
export async function POST(request: NextRequest, context: { params: { id: string } }) {
  ...
  const targetUrl = `${baseUrl}/api/agent/chatkit/session/${encodeURIComponent(chatkitSessionId)}/resume`;
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to resume ChatKit session';
