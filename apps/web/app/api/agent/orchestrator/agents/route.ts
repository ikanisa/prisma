...
export async function GET(request: NextRequest) {
  ...
  const targetUrl = `${baseUrl}/api/agent/orchestrator/agents${request.nextUrl.search}`;
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to proxy orchestrator agents request';
