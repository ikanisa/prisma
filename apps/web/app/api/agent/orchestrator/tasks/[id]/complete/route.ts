...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const targetUrl = `${baseUrl}/api/agent/orchestrator/tasks/${encodeURIComponent(params.id)}/complete`;
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to proxy orchestrator task completion request';
