...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const targetUrl = `${baseUrl}/api/agent/orchestrator/session/${encodeURIComponent(params.id)}`;
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to proxy orchestrator session board request';
