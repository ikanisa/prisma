...
export async function POST(request: NextRequest) {
  ...
  const targetUrl = `${baseUrl}/api/agent/domain-tools/image-generation`;
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to proxy image generation request';
