...
export default function AgentChat() {
  ...
  const [responseModel, setResponseModel] = useState('gpt-4.1-mini');
  const [responseMessages, setResponseMessages] = useState<ResponseMessageDraft[]>(() => {
    if (typeof window !== 'undefined') {
      const stored = parseStoredValue(window.localStorage.getItem(RESPONSE_MESSAGES_STORAGE_KEY));
      if (stored && Array.isArray(stored)) {
        return (stored as unknown[]).map((entry) => sanitizeStoredMessage(entry));
      }
    }
    return [createMessageDraft({ content: 'Provide a concise audit planning checklist.' })];
  });
  const [responseRequestJson, setResponseRequestJson] = useState('{"temperature":0.2}');
  const [responseToolOutputs, setResponseToolOutputs] = useState<ResponseToolOutputDraft[]>(() => {
    if (typeof window !== 'undefined') {
      const stored = parseStoredValue(window.localStorage.getItem(RESPONSE_TOOL_OUTPUTS_STORAGE_KEY));
      if (stored && Array.isArray(stored)) {
        return (stored as unknown[]).map((entry) => sanitizeStoredToolOutput(entry));
      }
    }
    return [];
  });
