...
const FUNCTIONS_BASE_URL = baseUrl ? `${baseUrl}/functions/v1/audit-kam` : '';
...
export function upsertDraft(payload: {
  orgSlug: string;
  engagementId: string;
  candidateId: string;
  heading: string;
  ...
  procedures?: Array<{ procedureId: string; isaRefs: string[] }>;
  evidence?: Array<{ evidenceId?: string; documentId?: string; note?: string }>;
}) {
  return request<{ draft: KamDraft }>(`/draft/upsert`, {
    method: 'POST',
    body: JSON.stringify(payload),
  });
}

export function submitDraft(payload: { orgSlug: string; engagementId: string; draftId: string }) {
  return request<{ approvalQueue: ApprovalQueueItem[] }>(`/draft/submit`, {
    method: 'POST',
    body: JSON.stringify(payload),
  });
}
...
export function resolveApproval(payload: {
  ...
  return request<{ approval: ApprovalQueueItem }>(`/approval/resolve`, {
    method: 'POST',
    body: JSON.stringify(payload),
  });
}
