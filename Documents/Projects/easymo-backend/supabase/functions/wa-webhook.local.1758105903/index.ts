...
async function waSend(path, payload) {
...
function safeRowTitle(t, max = 24) {
  if (!t) return "";
  if (t.length <= max) return t;
  return t.slice(0, max - 1) + "…";
}
function safeRowDesc(t, max = 72) {
  const s = (t ?? "").toString();
  if (s.length <= max) return s;
  return s.slice(0, max - 1) + "…";
}
function safeButtonTitle(t, max = 20) {
  if (t.length <= max) return t;
  return t.slice(0, max - 1) + "…";
}
    ...
    interactive: {
      ...
      action: {
        buttons: buttons.slice(0, 3).map((b)=>({
            ...
            reply: {
              id: b.id,
              title: safeButtonTitle(b.title, 20)
            }
          }))
      }
    }
...
async function sendList(to, opts) {
  return waSend("messages", {
    ...
    interactive: {
      ...
      header: {
        type: "text",
        text: safeRowTitle(opts.title, 60)
      },
      body: {
        text: opts.body.slice(0, 1024)
      },
      footer: {
        text: "Tip: Reply 'home' anytime."
      },
      action: {
...
async function sendImageUrl(to, link, caption) {
...
async function verifySignature(req, raw) {
  ...
  const key = await crypto.subtle.importKey("raw", enc.encode(WA_APP_SECRET), {
    name: "HMAC",
    hash: "SHA-256"
  }, false, [
    "sign"
...
async function ensureProfile(phone) {
  const { data } = await sb.from("profiles").select("*").eq("whatsapp_e164", phone).maybeSingle();
  if (data) return data;
...
async function idempotent(mid) {
  const got = await sb.from("wa_events").select("wa_message_id").eq("wa_message_id", mid).maybeSingle();
  if (got.data) return false;
  await sb.from("wa_events").insert({
    wa_message_id: mid
  });
  return true;
}
async function getState(uid) {
  const { data } = await sb.from("chat_state").select("state").eq("user_id", uid).maybeSingle();
  const raw = data?.state ?? null;
  if (!raw) return {
    key: "home",
    data: {}
  };
  ...
  if (typeof raw === "object" && raw.key) return {
    key: raw.key,
    data: raw.data ?? {}
  };
  return {
...
async function setState(uid, key, data = {}) {
...
async function getConfig() {
  const { data } = await sb.from("app_config").select("insurance_admin_numbers, momo_qr_logo_url").eq("id", 1).maybeSingle();
  const arr = data?.insurance_admin_numbers ?? [];
  const logo = data?.momo_qr_logo_url ?? null;
  return {
    insurance_admin_numbers: arr,
    momo_qr_logo_url: logo
  };
}
// --------- WA media fetch ----------
async function fetchWAMedia(metaId) {
...
async function runInsuranceOCR(url) {
  ...
  const body = {
    ...
    messages: [
      {
        role: "system",
        content: "Extract Rwanda motor insurance fields as JSON only. Keys: " + "insurer_name, policy_number, certificate_number, policy_inception, policy_expiry, " + "carte_jaune_number, carte_jaune_expiry, make, model, vehicle_year, registration_plate, " + "vin_chassis, licensed_to_carry."
      },
      {
        role: "user",
        content: [
...
function normIns(e) {
  ...
  if (o.registration_plate) o.registration_plate = up(String(o.registration_plate)).replace(/[^A-Z0-9]/g, "");
...
function insUserSummary(e) {
  const a = [
    e.make || e.model ? `Vehicle: ${e.make ?? ""} ${e.model ?? ""}${e.vehicle_year ? ` (${e.vehicle_year})` : ""}`.trim() : "Vehicle: (not detected)",
...
function insAdminSummary(user, e) {
  const p = [
...
// --------- RPCs (existing) ----------
async function rpcNearbyDrivers(lat, lon, viewer, limit = 10) {
...
async function rpcNearbyPassengers(lat, lon, viewer, limit = 10) {
...
async function rpcNearbyBusinesses(lat, lon, viewer, limit = 10) {
...
async function rpcMatchDriversForTrip(tripId, limit = 10) {
...
async function rpcMatchPassengersForTrip(tripId, limit = 10) {
...
async function rpcNearbyDriversByVehicle(lat, lon, viewer, vehicle, limit = 10) {
  ...
  try {
    ...
    const meta = ids.length ? await sb.from("profiles").select("user_id,meta").in("user_id", ids) : {
      data: []
    };
    const vehMap = new Map();
    (meta.data || []).forEach((m)=>{
      const v = m?.meta?.vehicle_type || m?.meta?.vehicle || null;
...
async function rpcNearbyPassengersByVehicle(lat, lon, viewer, vehicle, limit = 10) {
  try {
    ...
    const trips = tripIds.length ? await sb.from("trips").select("id,vehicle_type").in("id", tripIds) : {
      data: []
    };
    const vehMap = new Map();
    (trips.data || []).forEach((t)=>vehMap.set(t.id, t.vehicle_type || null));
    const want = vehicle.toLowerCase();
...
async function rpcMatchDriversForTripEx(tripId, limit = 10, preferDrop = true) {
  try {
    const { data, error } = await sb.rpc("match_drivers_for_trip", {
      _trip_id: tripId,
      _limit: limit,
...
async function rpcMatchPassengersForTripEx(tripId, limit = 10, preferDrop = true) {
  try {
    const { data, error } = await sb.rpc("match_passengers_for_trip", {
      _trip_id: tripId,
      _limit: limit,
...
async function markServedDriver(viewerMsisdn, driverUserId) {
...
async function markServedPassenger(viewerMsisdn, tripId) {
  const expires = new Date(Date.now() + 15 * 60 * 1000).toISOString();
  try {
...
function buildMomoUssd(target, isCode, amount) {
  // USSD rules per your plan
  const clean = target.replace(/\D/g, "");
  if (isCode) {
    return amount && amount > 0 ? `*182*8*1*${clean}*${amount}#` : `*182*8*1*${clean}#`;
  } else {
    // number path
    return amount && amount > 0 ? `*182*1*1*${clean}*${amount}#` : `*182*1*1*${clean}#`;
  }
}
...
async function isMember(basketId, userId) {
  const { data } = await sb.from("basket_members").select("basket_id").eq("basket_id", basketId).eq("user_id", userId).maybeSingle();
  return !!data;
}
async function addMember(basketId, userId) {
  try {
...
async function removeMember(basketId, userId) {
  try {
...
async function listMembersPage(basketId, page, pageSize) {
  const offset = page * pageSize;
  const { data, error } = await sb.from("basket_members").select("user_id,total_contributed").eq("basket_id", basketId).order("total_contributed", {
    ascending: false
...
function basketActionsRows(b, isCreator, isMemberNow) {
  const rows = [
    {
      id: `bk_det_${b.id}`,
    ...
    {
      id: `bk_share_${b.id}`,
      title: "Copy Basket Link"
    },
    {
      id: `bk_qr_${b.id}`,
      title: "Basket QR Code"
    },
    {
      id: `bk_mems_${b.id}_0`,
      title: "View Members (page 1)"
    },
    // NEW (additive) deeplink entries that won’t break old flows:
    {
      id: `bkdl_share_${b.id}`,
      title: "Share Basket (WhatsApp)"
    },
    {
      id: `bkdl_qr_${b.id}`,
      title: "QR to Open (WhatsApp)"
    }
  ];
  if (!isMemberNow) rows.push({
    id: `bk_join_${b.id}`,
    title: "Join Basket"
  });
  if (isMemberNow && !isCreator) rows.push({
    id: `bk_leave_${b.id}`,
    title: "Leave Basket"
  });
  if (isCreator) rows.push({
    id: `bk_close_${b.id}`,
    title: "Close Basket"
  });
  return rows;
}
function basketDetailsText(b) {
  const momo = b.momo_target ? b.momo_is_code ? `MoMo Code: ${b.momo_target}` : `MoMo Number: ${b.momo_target}` : "No MoMo set";
  return [
    `Name: ${b.name}`,
...
function qrUrlForJoin(slugOrId) {
...
function normalizeMoMoNumberInput(s) {
...
function normalizeMoMoCodeInput(s) {
  const d = s.replace(/\D/g, "");
  if (d.length >= 4 && d.length <= 9) return d;
  return null;
}
function buildUssdFor(kind, value, amount) {
...
function qrUrlForUssdTel(telUri, size = 768) {
...
function qrUrlForDeepLink(url, size = 512) {
  return `https://quickchart.io/qr?text=${encodeURIComponent(url)}&margin=1&size=${size}`;
}
async function createPendingContribution(basket_id, contributor_user_id, amount_rwf) {
  const ins = await sb.from("basket_contributions").insert({
    basket_id,
    contributor_user_id,
    amount_rwf,
    status: "pending",
...
async function approveContribution(contribId, approver_user_id) {
  const now = new Date().toISOString();
  const upd = await sb.from("basket_contributions").update({
    status: "approved",
    approved_at: now,
    approver_user_id
...
async function rejectContribution(contribId, approver_user_id) {
  const upd = await sb.from("basket_contributions").update({
    status: "rejected",
    approver_user_id
  }).eq("id", contribId).select("*").single();
...
async function getContribution(contribId) {
  const { data } = await sb.from("basket_contributions").select("*").eq("id", contribId).maybeSingle();
  return data || null;
}
// Keep basket_members totals in sync after approval (additive)
async function bumpMemberTotal(basketId, userId, amount) {
  const { data: row } = await sb.from("basket_members").select("total_contributed").eq("basket_id", basketId).eq("user_id", userId).maybeSingle();
...
async function sendPublicReviewCardToAdmin(b, creatorWaE164) {
  try {
...
Slug: ${b.public_slug ?? "(none)"}`;
    ...
    await sendButtons(e164(BASKET_REVIEW_ADMIN), "Basket review", [
      {
        id: `pubrev_appr_${b.id}`,
        title: "Approve"
      },
      {
        id: `pubrev_chat_${digits}`,
        title: "Chat creator"
      },
      {
        id: `pubrev_view_${b.id}`,
        title: "View basket"
      }
    ]);
  } catch (e) {
...
async function notifyCreatorApproved(creatorWaE164, b) {
  try {
...
async function updateTripDropoff(tripId, lon, lat) {
  try {
...
serve(async (req)=>{
  ...
  if (req.method === "GET") {
    ...
    if (u.searchParams.get("hub.mode") === "subscribe" && u.searchParams.get("hub.verify_token") === WA_VERIFY_TOKEN) {
  ...
  if (!await verifySignature(req, raw)) return new Response("Bad sig", {
    status: 401
  });
  let payload;
  try {
  ...
  if (msg.interactive?.type === "list_reply") {
    ...
    if (id === "see_drivers") {
      ...
      await setState(user_id, "near_await_vehicle_choice", {
        mode: "drivers"
      });
      await sendList(from, {
        title: "Nearby Drivers",
    ...
    if (nearDrvPrefix.test(id)) {
      ...
      await setState(user_id, "near_await_loc_drivers", {
        vehicle_type
      });
      await sendButtons(from, "Share your current location to see nearby drivers.\n\nTap ➕ and share your current location.", [
        {
          id: "back_home",
          title: "Back to Menu"
        }
    ...
    if (nearPaxPrefix.test(id)) {
      ...
      await setState(user_id, "near_await_loc_passengers", {
        vehicle_type
      });
      await sendButtons(from, "Share your current location to see nearby passengers.\n\nTap ➕ and share your current location.", [
        {
          id: "back_home",
          title: "Back to Menu"
        }
    ...
    if (id === "bk_type_public") {
      ...
      await setState(user_id, "await_basket_momo", {
        name,
        description,
        type: "public"
      });
    ...
    if (id.startsWith("bkdl_")) {
      ...
      if (!b) {
        await sendButtons(from, "Basket not available.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
  ...
  if (msg.interactive?.type === "button_reply") {
    ...
    if (bid.startsWith("bk_appr_") || bid.startsWith("bk_rej_")) {
      ...
      if (!basketId || !memberId || !Number.isFinite(amountCents)) {
        await sendButtons(from, "Invalid action button.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      if (action === "appr") {
        try {
          const { data: row } = await sb.from("basket_members").select("total_contributed").eq("basket_id", basketId).eq("user_id", memberId).maybeSingle();
          ...
          const { data: prof } = await sb.from("profiles").select("whatsapp_e164").eq("user_id", memberId).maybeSingle();
          const memberWa = prof?.whatsapp_e164;
          if (memberWa) {
            await sendText(memberWa, `✅ Contribution approved: RWF ${amountRwf.toLocaleString()}. Thank you!`);
          }
          await sendButtons(from, "Approved and member notified.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
          ...
          await sendButtons(from, "Couldn’t approve. Try again.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
      ...
      if (action === "rej") {
        try {
          ...
          await sendButtons(from, "Rejected and member notified.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
          ...
          await sendButtons(from, "Couldn’t reject. Try again.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
    ...
    if (bid.startsWith("pubrev_")) {
      ...
      try {
        if (action === "appr") {
          ...
          await sendButtons(from, "✅ Basket approved.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
        ...
        await sendButtons(from, "Action failed.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
    ...
    if (bid.startsWith("contrib_")) {
      ...
      try {
        if (action === "appr") {
          ...
          const { data: prof } = await sb.from("profiles").select("whatsapp_e164").eq("user_id", row.contributor_user_id).maybeSingle();
          if (prof?.whatsapp_e164) {
            await sendText(prof.whatsapp_e164, `✅ Contribution approved: RWF ${row.amount_rwf.toLocaleString()}. Thank you!`);
          }
          await sendButtons(from, "Approved & contributor notified.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
        ...
        if (action === "rej") {
          ...
          await sendButtons(from, "Rejected & contributor notified.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
  ...
  /* =========================
   ...
   ========================= */ /* deno-lint-ignore-file no-explicit-any */ // ------------------ CONTINUATION: LIST REPLIES (existing + additive) ------------------
  if (msg.interactive?.type === "list_reply") {
    const id = msg.interactive.list_reply.id || "";
    ...
    if (id === "motor_insurance") {
      ...
      await sendButtons(from, "Send a photo or PDF of your insurance certificate or yellow vehicle card (one file only).\n\nTap ➕ and upload your file.", [
        {
          id: "back_home",
          title: "Back to Menu"
        }
    ...
    if (id === "role_passenger" || id === "role_driver") {
      ...
      await setState(user_id, "await_schedule_vehicle", {
        role
      });
      await sendList(from, {
        title: "Vehicle Type",
    ...
    if (id === "mk_add") {
      const { data: cats } = await sb.from("marketplace_categories").select("id,name,sort_order,is_active").eq("is_active", true).order("sort_order", {
        ascending: true
      }).order("name", {
        ascending: true
      });
    ...
    if (id.startsWith("cat_")) {
      ...
      await setState(user_id, "await_business_name", {
        category_id
      });
    ...
    if (id.startsWith("see_cat_")) {
      ...
      await sendButtons(from, "Share your location to find nearby businesses.\n\nTap ➕ and share your current location.", [
        {
          id: "back_home",
          title: "Back to Menu"
        }
    ...
    if (id.startsWith("veh_") && state === "await_schedule_vehicle") {
      ...
      await setState(user_id, "await_schedule_pickup", {
        role,
        vehicle_type
      });
    ...
    if (id === "bk_view") {
      const mineIds = [];
      const created = await sb.from("baskets").select("id,name,created_at").eq("creator_id", user_id).order("created_at", {
        ascending: false
      }).limit(10);
      (created.data || []).forEach((b)=>mineIds.push(b.id));
      const memberOf = await sb.from("basket_members").select("basket_id").eq("user_id", user_id).limit(50);
      (memberOf.data || []).forEach((m)=>{
        if (!mineIds.includes(m.basket_id)) mineIds.push(m.basket_id);
      });
      let rows = [];
      if (mineIds.length) {
        const { data: bs } = await sb.from("baskets").select("id,name,type,status,created_at").in("id", mineIds).order("created_at", {
          ascending: false
        }).limit(10);
        rows = (bs || []).map((b)=>({
            id: `b_${b.id}`,
        ...
        const pub = await sb.from("baskets").select("id,name,description,created_at").eq("type", "public").eq("status", "approved").order("created_at", {
          ascending: false
        }).limit(10);
        rows = (pub.data || []).map((b)=>({
            id: `b_${b.id}`,
      ...
      if (!rows.length) {
        await sendButtons(from, "No baskets yet.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
    ...
    if (id === "bk_create") {
      ...
      await sendButtons(from, "What’s the basket name?", [
        {
          id: "back_home",
          title: "Back to Menu"
        }
  ...
  if (msg.type === "text" && msg.text?.body) {
    ...
    if (state === "await_business_name") {
      await setState(user_id, "await_business_desc", {
        ...data,
        name: body
      });
    ...
    if (state === "await_business_desc") {
      await setState(user_id, "await_business_catalog", {
        ...data,
        description: body
      });
      await sendButtons(from, "Paste your WhatsApp catalog link (http…) or tap Skip.", [
        {
          id: "biz_catalog_skip",
          title: "Skip"
        },
    ...
    if (state === "await_business_catalog") {
      ...
      await setState(user_id, "await_business_location", {
        ...data,
        catalog_url
      });
      await sendButtons(from, "Share your business location.\n\nTap ➕ and share your current location.", [
        {
          id: "back_home",
          title: "Back to Menu"
        }
    ...
    if (state === "await_basket_name") {
      await setState(user_id, "await_basket_desc", {
        name: body
      });
    ...
    if (state === "await_basket_desc") {
      await setState(user_id, "await_basket_type", {
        ...data,
        description: body
      });
      await sendList(from, {
    ...
    if (state === "await_basket_momo") {
      ...
      await setState(user_id, "await_basket_confirm", {
        ...data,
        momo_target,
        momo_is_code
      });
      await sendButtons(from, `Saved.\n${momo_is_code ? "MoMo Code" : "MoMo Number"}: ${momo_target}\nCreate basket now?`, [
        {
          id: "bk_do_create",
          title: "Create"
        },
    ...
    if (state === "await_contrib_amount") {
      ...
      if (!Number.isFinite(amount) || amount <= 0) {
        await sendButtons(from, "Enter a valid amount (RWF).", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      const baskId = data?.basket_id;
      const bask = await getBasket(baskId);
      if (!bask) {
        await clearState(user_id);
        await sendButtons(from, "Basket not found.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      if (creatorWa) {
        if (pending?.id) {
          ...
          await sendButtons(creatorWa, `Contribution review\nBasket: ${bask.name}\nFrom: ${from}\nAmount: RWF ${amount.toLocaleString()}`, [
            {
              id: `contrib_appr_${pending.id}`,
              title: "Approve"
            },
            {
              id: `contrib_rej_${pending.id}`,
              title: "Reject"
            },
            {
              id: `contrib_chat_${toWaDigits(from)}`,
              title: "Chat"
            }
          ]);
        } else {
          ...
          await sendButtons(creatorWa, `Contribution review:\nBasket: ${bask.name}\nFrom: ${from}\nAmount: RWF ${amount.toLocaleString()}`, [
            {
              id: `bk_appr_${bask.id}_${user_id}_${cents}`,
              title: "Approve"
            },
            {
              id: `bk_rej_${bask.id}_${user_id}_${cents}`,
              title: "Reject"
            }
          ]);
        }
    ...
    if (state === "momoqr_await_number") {
      ...
      if (!normalized) {
        await sendButtons(from, "Send your MoMo number (07…).", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      await setState(user_id, "momoqr_await_amount", {
        kind: "number",
        momo_value: normalized
      });
    ...
    if (state === "momoqr_await_code") {
      ...
      if (!normalized) {
        await sendButtons(from, "Send your MoMo code (4–9 digits).", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
  ...
  if (msg.interactive?.type === "button_reply") {
    ...
    if (bid === "bk_do_create" && state === "await_basket_confirm") {
      ...
      try {
        ...
        const ins = await sb.from("baskets").insert({
          name,
          description,
          type,
        ...
        if (type === "public") {
          ...
          await sendButtons(from, "Basket created and sent for review.\nMeanwhile you can preview/share:", [
            {
              id: `bkdl_share_${ins.data.id}`,
              title: "Share Basket (WhatsApp)"
            },
            {
              id: `bkdl_qr_${ins.data.id}`,
              title: "QR to Open (WhatsApp)"
            }
          ]);
        ...
        console.error(e);
        await sendButtons(from, "Couldn’t create basket.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
        ...
        rows: [
          {
            ...
            description: `Your WhatsApp: ${my07 || "unknown"}`
          },
          {
            id: "mqr_enter_num",
            title: "Enter MoMo number",
            description: "07… mobile number"
          },
          {
            id: "mqr_enter_code",
            title: "Enter MoMo code",
            description: "4–9 digit code"
          }
        ]
      });
      return new Response("ok");
    ...
    if (bid === "sched_add_drop" && state === "sched_post_pickup") {
      ...
      if (!trip_id) {
        await clearState(user_id);
        await sendButtons(from, "Trip context lost. Please schedule again.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      await setState(user_id, "sched_await_drop", {
        trip_id,
        role
      });
    ...
    if (bid === "sched_skip_drop" && state === "sched_post_pickup") {
      ...
      try {
        if (role === "passenger") {
          ...
          if (matches?.length) {
            await setState(user_id, "await_match_select", {
              role,
              list: matches
            });
            await sendList(from, {
            ...
            await sendButtons(from, "No immediate matches. We’ll keep looking.", [
              {
                id: "back_home",
                title: "Back to Menu"
              }
        ...
        await sendButtons(from, "Couldn’t fetch matches.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
  ...
  if (msg.type === "location") {
    ...
    if (state === "near_await_loc_drivers") {
      try {
        const veh = data?.vehicle_type || "moto";
        const rows = await rpcNearbyDriversByVehicle(lat, lon, from, veh, 10);
        if (!rows?.length) {
          await sendButtons(from, "No drivers nearby for that vehicle type.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
        ...
        console.error(e);
        await sendButtons(from, "Couldn’t fetch drivers.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
    ...
    if (state === "near_await_loc_passengers") {
      try {
        ...
        if (!rows?.length) {
          await sendButtons(from, "No passengers nearby for that vehicle type.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
        ...
        console.error(e);
        await sendButtons(from, "Couldn’t fetch passengers.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
    ...
    if (state === "await_market_see_loc") {
      const st2 = await getState(user_id);
      const catId = st2?.data?.category_id;
      try {
        ...
        if (catId) {
          ...
          if (ids.length) {
            ...
            const idToCat = new Map(meta?.map((m)=>[
                m.id,
                m.category_id
              ]) || []);
        ...
        console.error(e);
        await sendButtons(from, "Couldn’t fetch businesses.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
    ...
    if (state === "await_schedule_pickup") {
      ...
      try {
        ...
        if (ins.error) throw ins.error;
        const newTripId = ins.data.id;
        // Persist a transient state to handle the drop-off choice
        await setState(user_id, "sched_post_pickup", {
          trip_id: newTripId,
          role,
          vehicle_type
        });
      ...
      // (We’ll only fetch/send matches after the user chooses add/skip and we either get dropoff or skip.)
        ...
        console.error(e);
        await sendButtons(from, "Couldn’t save your trip.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
  ...
  if (msg.interactive?.type === "list_reply") {
    ...
    if (id.startsWith("drv_")) {
      ...
      if (!chosen) {
        await sendButtons(from, "That driver is no longer available.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      if (!wa) {
        await sendButtons(from, "No contact for this driver.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
    ...
    if (id.startsWith("pax_")) {
      ...
      if (!wa && chosen.creator_user_id) {
        const p = await sb.from("profiles").select("whatsapp_e164").eq("user_id", chosen.creator_user_id).maybeSingle();
        wa = p.data?.whatsapp_e164;
      }
      if (!wa) {
        await sendButtons(from, "No contact for this passenger.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
    ...
    if (id.startsWith("biz_")) {
      ...
      const chosen = arr[idx];
      if (!chosen) {
        await sendButtons(from, "Business not available.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      const biz = await sb.from("businesses").select("owner_whatsapp,catalog_url,name,id").eq("id", chosen.id).maybeSingle();
      if (!biz.data) {
        await sendButtons(from, "Business not available.", [
          {
            id: "back_home",
      ...
      const btns = [];
      if (catalog_url) btns.push({
        id: `biz_catalog_${realId}`,
        title: "View Catalog"
      });
      if (owner_whatsapp) btns.push({
        id: `biz_contact_${realId}`,
        title: "Contact"
      });
    ...
    if (id.startsWith("b_")) {
      ...
      await setState(user_id, "basket_ctx", {
        basket_id: basketId
      });
      await sendList(from, {
    ...
    if (id.startsWith("bk_")) {
      ...
      if (id.startsWith("bk_det_")) {
        await sendButtons(from, basketDetailsText(b), [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      if (id.startsWith("bk_leave_")) {
        ...
        await sendButtons(from, "You left the basket.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      if (id.startsWith("bk_close_")) {
        if (!isCreator) {
          await sendButtons(from, "Only the creator can close this basket.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
        ...
        try {
          ...
          await sendButtons(from, "Basket closed.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
          ...
          console.error(e);
          await sendButtons(from, "Couldn’t close the basket.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
      ...
      if (id.startsWith("bk_cont_")) {
        if (b.momo_target) {
          ...
          await sendButtons(from, "Creator didn’t set MoMo yet.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
        ...
        await setState(user_id, "await_contrib_amount", {
          basket_id: b.id
        });
      ...
      if (id.startsWith("bk_mems_")) {
        ...
        if (!members.length) {
          await sendButtons(from, "No more members.", [
            {
              id: "back_home",
              title: "Back to Menu"
            }
    ...
    if (id.startsWith("mtch_")) {
      ...
      if (!tripRow) {
        await sendButtons(from, "No longer available.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
      ...
      const { data: prof } = await sb.from("profiles").select("whatsapp_e164").eq("user_id", tripRow.creator_user_id).maybeSingle();
      const wa = prof?.whatsapp_e164;
      if (!wa) {
        await sendButtons(from, "No contact found.", [
          {
            id: "back_home",
            title: "Back to Menu"
          }
  ...
  if (state === "ins_wait_doc" && (msg.type === "image" || msg.type === "document")) {
    ...
    try {
      const leadIns = await sb.from("insurance_leads").insert({
        whatsapp: from
      }).select("id").single();
      if (leadIns.error) throw leadIns.error;
      const leadId = leadIns.data.id;
      ...
      console.error(err);
      await sendButtons(from, "We couldn’t read the file. Please send a clear photo or PDF (one file only).\n\nTap ➕ and upload your file.", [
        {
          id: "back_home",
          title: "Back to Menu"
        }
...
/** Generate & send MoMo QR (image + dial/share text + buttons), with logging */ async function generateAndSendMomoQR(args) {
...
// -----------------------------------------------------------------------------
/** Strip and return only E.164 digits (no +). */ function toWaDigits_DUP(e164like) {
  const s = e164(e164like);
  return s.replace(/\D/g, "");
}
/** Get the public WhatsApp number for deeplinks. Prefers ENV.WA_BOT_NUMBER_E164. */ function getBotNumberDigits() {
...
/** Build a WhatsApp deeplink to our bot with JOIN_BASKET:<token> prefilled. */ function basketDeepLink(token) {
  const digits = getBotNumberDigits();
  const base = digits ? `https://wa.me/${digits}` : `https://wa.me/`;
  const text = encodeURIComponent(`JOIN_BASKET:${token}`);
  return `${base}?text=${text}`;
}
/** QR URL that encodes any deeplink URL (for QuickChart). */ function qrUrlForDeepLink(url, size = 512) {
  return `https://quickchart.io/qr?text=${encodeURIComponent(url)}&margin=1&size=${size}`;
}
// -----------------------------------------------------------------------------
// Nearby — vehicle-aware RPC wrappers (additive)
// -----------------------------------------------------------------------------
/** NEW RPC: nearby_drivers_by_vehicle */ async function rpcNearbyDriversByVehicle(lat, lon, viewer, vehicle, limit = 10) {
  ...
  try {
    ...
    try {
      const { data: metas } = await sb.from("drivers").select("user_id,vehicle_type").in("user_id", ids);
      const map = new Map((metas || []).map((m)=>[
          m.user_id,
          m.vehicle_type
        ]));
...
/** NEW RPC: nearby_passengers_by_vehicle */ async function rpcNearbyPassengersByVehicle(lat, lon, viewer, vehicle, limit = 10) {
  try {
    const { data, error } = await sb.rpc("nearby_passengers_by_vehicle", {
      _lat: lat,
      _lon: lon,
    ...
    try {
      const { data: trips } = await sb.from("trips").select("id,vehicle_type").in("id", tripIds);
      const map = new Map((trips || []).map((t)=>[
          t.id,
          t.vehicle_type
        ]));
...
// -----------------------------------------------------------------------------
/** Extended driver match with optional preferDropoff scoring. */ async function rpcMatchDriversForTripEx(tripId, limit = 10, preferDropoff = false) {
  try {
    const { data, error } = await sb.rpc("match_drivers_for_trip", {
      _trip_id: tripId,
      _limit: limit,
...
/** Extended passenger match with optional preferDropoff scoring. */ async function rpcMatchPassengersForTripEx(tripId, limit = 10, preferDropoff = false) {
  try {
    const { data, error } = await sb.rpc("match_passengers_for_trip", {
      _trip_id: tripId,
      _limit: limit,
...
/** Update a trip with a dropoff point (geometry Point, SRID 4326). */ async function updateTripDropoff(tripId, lon, lat) {
  await sb.from("trips").update({
    dropoff: `SRID=4326;POINT(${lon} ${lat})`
  }).eq("id", tripId);
}
/** Create a pending contribution row. */ async function createPendingContribution(basketId, contributorUserId, amountRwf) {
  const ins = await sb.from("basket_contributions").insert({
    basket_id: basketId,
...
/** Approve a pending contribution: set status, stamp approved_at, update member total. */ async function approveContribution(contribId, approverUserId) {
  // Fetch the row first
  const { data: row, error: err1 } = await sb.from("basket_contributions").select("*").eq("id", contribId).maybeSingle();
  if (err1) throw err1;
  if (!row) return null;
  if (row.status === "approved") return row; // idempotent
...
/** Reject a pending contribution. */ async function rejectContribution(contribId, approverUserId) {
  const { data: row, error: err1 } = await sb.from("basket_contributions").select("*").eq("id", contribId).maybeSingle();
  if (err1) throw err1;
  if (!row) return null;
  if (row.status === "rejected") return row; // idempotent
  if (row.status === "approved") return row; // do nothing
  const upd = await sb.from("basket_contributions").update({
    status: "rejected",
    approved_at: null,
    approver_user_id: approverUserId
...
/** Increment or initialize member total on approval. */ async function incrementMemberTotal(basketId, userId, amountRwf) {
  const { data: row } = await sb.from("basket_members").select("total_contributed").eq("basket_id", basketId).eq("user_id", userId).maybeSingle();
  const prev = row?.total_contributed ?? 0;
  await sb.from("basket_members").upsert({
    basket_id: basketId,
...
/** Notify contributor of approval/rejection (best-effort). */ async function notifyContributorOutcome(contrib) {
  try {
    const { data: prof } = await sb.from("profiles").select("whatsapp_e164").eq("user_id", contrib.contributor_user_id).maybeSingle();
    const wa = prof?.whatsapp_e164;
    if (!wa) return;
...
// -----------------------------------------------------------------------------
/** Send an interactive review card to the admin for public basket. */ async function sendPublicReviewCardToAdmin(basketRow, creatorE164) {
  const name = basketRow?.name || "(unnamed)";
  const slug = basketRow?.public_slug || basketRow?.id || "";
  const body = `Basket review\nName: ${name}\nType: Public\nCreator: ${creatorE164}\nSlug: ${slug}`;
  const rows = [
    {
      id: `pubrev_appr_${basketRow.id}`,
      title: "Approve"
    },
    {
      id: `pubrev_chat_${toWaDigits(creatorE164)}`,
      title: "Chat creator"
    },
    {
      id: `pubrev_view_${basketRow.id}`,
      title: "View basket"
    }
  ];
  await sendButtons(e164(BASKET_REVIEW_ADMIN), body, rows);
}
/** Approve a public basket (status=approved). */ async function approvePublicBasket(basketId) {
  await sb.from("baskets").update({
    status: "approved"
  }).eq("id", basketId);
}
/** Notify creator after basket approval with Share/QR buttons. */ async function notifyCreatorApproved(basketId) {
  const b = await getBasket(basketId);
  if (!b) return;
  const { data: prof } = await sb.from("profiles").select("whatsapp_e164").eq("user_id", b.creator_id).maybeSingle();
  const wa = prof?.whatsapp_e164;
...
/** Build a QR for the deeplinked basket open. */ function basketDeepLinkQrUrl(basket) {
...
// -----------------------------------------------------------------------------
/** Handle Share Basket (WhatsApp) button for any user. */ async function handleBasketShareDeeplink(to, basketId) {
  const b = await getBasket(basketId);
  if (!b) {
    await sendButtons(to, "Basket not available.", [
      {
        id: "back_home",
        title: "Back to Menu"
      }
...
/** Handle QR to Open (WhatsApp) button for any user. */ async function handleBasketQrDeeplink(to, basketId) {
  const b = await getBasket(basketId);
  if (!b) {
    await sendButtons(to, "Basket not available.", [
      {
...
// -----------------------------------------------------------------------------
async function sendVehicleChoiceForDrivers(to) {
  await sendList(to, {
    title: "Nearby Drivers",
    body: "Which vehicle type do you want to see nearby?",
    sectionTitle: "Vehicle",
...
async function sendVehicleChoiceForPassengers(to) {
  await sendList(to, {
    title: "Nearby Passengers",
    body: "Which vehicle type do you want to see nearby?",
    sectionTitle: "Vehicle",
...
/** Map our near_v_* keys to canonical vehicle_type values used in DB. */ function mapNearVehicleIdToType(nearId) {
...
// -----------------------------------------------------------------------------
async function recomputeBasketTotalsIfPresent(basketId) {
